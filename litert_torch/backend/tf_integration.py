...
def _wrap_as_tf_func(lowered, tf_state_dict):
  ...
  # TFXLA does not register arith dialect. Therefore replace arith.constant with
  # stablehlo.constant to run the module with TFXLA.
  # The string replacement is a hack and should be replaced with a proper pass.
  # However, tf_integration is only for testing purpose and never used in prod,
  # so this should be fine.
  ir_text = lowered.module.operation.get_asm()
  ir_text = ir_text.replace("arith.constant", "stablehlo.constant")
